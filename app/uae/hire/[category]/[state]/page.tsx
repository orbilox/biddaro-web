...
import {
  UAE_JOB_CATEGORY_META, UAE_LOCATIONS,
  getUAECategory, getUAELocationBySlug,
} from '@/lib/seo-data-uae';

interface Props {
  params: { category: string; state: string };
}

// ── Static params — categories × emirates ────────────────────────────────────
export function generateStaticParams() {
  return UAE_JOB_CATEGORY_META.flatMap((cat) =>
    UAE_LOCATIONS.map((loc) => ({ category: cat.slug, state: loc.slug }))
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const loc = getUAELocationBySlug(params.state);
  if (!cat || !loc) return {};

  const title = `${cat.plural} in ${loc.name} \u2013 Hire Top ${cat.name} Contractors | Biddaro`;
  const description = `Find verified ${cat.plural.toLowerCase()} in ${loc.name}, UAE. Post your ${cat.name.toLowerCase()} job free on Biddaro and receive competitive bids within hours. Secure escrow payments included.`;
  const url = `https://biddaro.com/uae/hire/${cat.slug}/${loc.slug}`;

  return {
    title,
    description,
    keywords: [
      `${cat.plural.toLowerCase()} in ${loc.name}`,
      `${cat.name.toLowerCase()} contractors ${loc.name}`,
      `hire ${cat.plural.toLowerCase()} ${loc.name}`,
      `${cat.plural.toLowerCase()} near me ${loc.name}`,
      `${cat.name.toLowerCase()} services ${loc.capital}`,
      `best ${cat.plural.toLowerCase()} ${loc.name}`,
      'Biddaro contractors UAE',
    ],
...
// ── JSON-LD schemas ──────────────────────────────────────────────────────────
function JsonLd({ cat, loc }: { cat: NonNullable<ReturnType<typeof getUAECategory>>; loc: NonNullable<ReturnType<typeof getUAELocationBySlug>> }) {
  const url = `https://biddaro.com/uae/hire/${cat.slug}/${loc.slug}`;
  ...
  const schema = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'UAE Contractors', item: 'https://biddaro.com/uae/hire' },
...
function getDummyContractors(cat: NonNullable<ReturnType<typeof getUAECategory>>, loc: NonNullable<ReturnType<typeof getUAELocationBySlug>>) {
  const names = [
    'Al Jazeera Construction', 'Emirates Build Masters', 'Gulf Pro Services',
    'Dar Construction', 'Nakheel Contractors', 'RAK Builders',
    'Masdar Works', 'Emaar Contractors', 'Sharjah Build Group', 'Al Ain Pro Services',
  ];
  const cities = loc.cities.slice(0, 6);
  return names.slice(0, 6).map((name, i) => ({
    id: i + 1,
    name,
    city: cities[i % cities.length]?.name ?? loc.capital,
...
function ContractorCard({ c }: { c: ReturnType<typeof getDummyContractors>[0] }) {
  return (
    <div className={`bg-white border rounded-xl p-5 hover:shadow-md transition-all group ${c.isPremium ? 'border-amber-300 ring-1 ring-amber-100 hover:border-amber-400' : 'border-gray-200 hover:border-brand-200'}`}>
      ...
      <h3 className="font-semibold text-dark-800 group-hover:text-brand-700 transition-colors mb-1 text-sm">{c.name}</h3>
...
// ── Main Page ────────────────────────────────────────────────────────────────
export default function UAEEmiratePage({ params }: Props) {
  const cat = getUAECategory(params.category);
  const loc = getUAELocationBySlug(params.state);
  if (!cat || !loc) notFound();

  const contractors = getDummyContractors(cat, loc);
  const otherCategories = UAE_JOB_CATEGORY_META.filter((c) => c.slug !== cat.slug).slice(0, 10);
  const otherEmirates = UAE_LOCATIONS.filter((l) => l.slug !== loc.slug);
    ...
    <>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-3" aria-label="Breadcrumb">
        <div className="max-w-7xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-dark-500 flex-wrap">
            ...
            <li><Link href="/uae/hire" className="hover:text-brand-600 transition-colors">UAE Contractors</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li><Link href={`/uae/hire/${cat.slug}`} className="hover:text-brand-600 transition-colors">{cat.plural}</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-brand-500/20 border border-brand-500/30 text-brand-300 text-xs font-medium px-3 py-1.5 rounded-full mb-5">
            <MapPin className="w-3 h-3" /> {loc.name}, UAE
          </div>
      ...
      <section className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            { icon: Users, value: '500+', label: 'Contractors' },
            { icon: Award, value: '4.8\u2605', label: 'Avg Rating' },
            { icon: Clock, value: '< 4 hrs', label: 'First Bid' },
            { icon: DollarSign, value: 'AED 0', label: 'To Post' },
      ...
      <section className="py-14 px-4 bg-gray-50">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              { icon: Shield, title: 'Verified Professionals', desc: `Every ${cat.name.toLowerCase()} contractor in ${loc.name} is ID-verified with work history reviewed before they can bid on your job.` },
              { icon: Zap, title: 'Competitive Bidding', desc: `Post once and receive multiple bids from ${cat.plural.toLowerCase()} in ${loc.name}. Compare pricing, experience and ratings to choose the best.` },
              { icon: DollarSign, title: 'Secure Escrow Payments', desc: `Your payment is held safely in escrow and released only when you approve completed work \u2014 zero risk of being scammed.` },
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-bold text-dark-900">Top {cat.plural} in {loc.name}</h2>
              <p className="text-dark-500 text-sm mt-1">Verified professionals ready to bid on your project</p>
            </div>
            <Link href="/register" className="hidden sm:inline-flex items-center gap-1.5 text-sm font-medium text-brand-600 hover:text-brand-700 transition-colors">
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">{cat.name} Services Available in {loc.name}</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {cat.skills.map((skill) => (
              <div key={skill} className="flex items-center gap-2.5 bg-white border border-gray-200 rounded-lg px-4 py-3">
          ...
          <div className="mt-6 p-4 bg-brand-50 border border-brand-100 rounded-xl text-sm text-brand-800">
            <strong>Average Rate in {loc.name}:</strong> {cat.avgRate} \u2014 rates may vary based on job complexity and location within the emirate.
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-2">{cat.plural} by Area in {loc.name}</h2>
          <p className="text-dark-500 text-sm mb-6">Find {cat.plural.toLowerCase()} in specific areas across {loc.name}.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {loc.cities.map((city) => (
              <Link
                key={city.slug}
                href={`/uae/hire/${cat.slug}/${loc.slug}/${city.slug}`}
                className="group flex items-center justify-between gap-2 bg-gray-50 border border-gray-200 hover:border-brand-300 hover:bg-brand-50 rounded-lg px-4 py-3 transition-all"
              >
                <div>
      ...
      {/* Other categories in this emirate */}
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">Other Contractors Available in {loc.name}</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {otherCategories.map((c) => (
              <Link key={c.slug} href={`/uae/hire/${c.slug}/${loc.slug}`}
                className="group flex items-center gap-2 bg-white border border-gray-200 hover:border-brand-300 hover:bg-brand-50 rounded-lg px-3 py-3 transition-all">
      ...
      {/* Same category in other emirates */}
      <section className="py-12 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">{cat.plural} in Other Emirates</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {otherEmirates.map((s) => (
              <Link key={s.slug} href={`/uae/hire/${cat.slug}/${s.slug}`}
      ...
      <section className="py-16 px-4 bg-gradient-to-r from-brand-500 to-brand-600">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-brand-100 text-lg mb-8">
            Post your job for free and receive bids from verified professionals in {loc.name} within hours.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/register" className="inline-flex items-center justify-center gap-2 bg-white text-brand-600 hover:bg-brand-50 font-bold px-10 py-4 rounded-xl transition-colors text-lg">
              Post a Job Free <ArrowRight className="w-5 h-5" />
            </Link>
            <Link href="/login" className="inline-flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 text-white font-semibold px-8 py-4 rounded-xl transition-colors border border-white/30">
              Sign In
            </Link>
          </div>
