...
import {
  ...
  getUAECategory, getUAELocationBySlug, getUAECity,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const city = getUAECity(params.state, params.city);
  ...
  const url = `https://biddaro.com/uae/hire/${cat.slug}/${loc.slug}/${city.slug}`;
...
function JsonLd({
  ...
  cat: NonNullable<ReturnType<typeof getUAECategory>>;
  loc: NonNullable<ReturnType<typeof getUAELocationBySlug>>;
  city: NonNullable<ReturnType<typeof getUAECity>>;
}) {
  const url = `https://biddaro.com/uae/hire/${cat.slug}/${loc.slug}/${city.slug}`;
  ...
  const schema = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 4, name: loc.name, item: `https://biddaro.com/uae/hire/${cat.slug}/${loc.slug}` },
    ...
    },
    {
      ...
      areaServed: {
        ...
        containedInPlace: { '@type': 'AdministrativeArea', name: loc.name, containedInPlace: { '@type': 'Country', name: 'United Arab Emirates' } },
      },
      url,
    },
...
function getDummyContractors(
  ...
  return [
    { id: 1, name: 'Al Jazeera Construction', rating: 4.9, reviews: 87, jobs: 156, verified: true, specialty: cat.skills[0] },
    { id: 2, name: 'Emirates Build Masters', rating: 4.7, reviews: 52, jobs: 93, verified: true, specialty: cat.skills[1 % cat.skills.length] },
    { id: 3, name: 'Gulf Pro Services', rating: 4.6, reviews: 34, jobs: 61, verified: true, specialty: cat.skills[2 % cat.skills.length] },
    { id: 4, name: 'Dar Construction', rating: 4.5, reviews: 28, jobs: 44, verified: false, specialty: cat.skills[3 % cat.skills.length] },
    { id: 5, name: 'Nakheel Contractors', rating: 4.8, reviews: 71, jobs: 118, verified: true, specialty: cat.skills[4 % cat.skills.length] },
    { id: 6, name: 'RAK Builders', rating: 4.4, reviews: 19, jobs: 32, verified: false, specialty: cat.skills[0] },
  ];
}

export default function UAEHireCityPage({ params }: Props) {
  const cat = getUAECategory(params.category);
  const loc = getUAELocationBySlug(params.state);
  const city = getUAECity(params.state, params.city);
  ...
  const otherCategories = UAE_JOB_CATEGORY_META.filter((c) => c.slug !== cat.slug).slice(0, 8);
  ...
  return (
    <>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-3" aria-label="Breadcrumb">
        <div className="max-w-7xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-dark-500 flex-wrap">
            ...
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li><Link href={`/uae/hire/${cat.slug}/${loc.slug}`} className="hover:text-brand-600 transition-colors">{loc.name}</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
      ...
      <section className="py-14 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-bold text-dark-900 mb-2">Verified {cat.plural} in {city.name}</h2>
          <p className="text-dark-500 text-sm mb-8">All contractors are verified, reviewed, and ready to bid on your project.</p>
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">{cat.name} Services in {city.name}</h2>
      ...
      {/* Other cities in emirate */}
      {otherCities.length > 0 && (
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-xl font-bold text-dark-900 mb-6">{cat.plural} in Other Areas of {loc.name}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {otherCities.map((c) => (
                <Link key={c.slug} href={`/uae/hire/${cat.slug}/${loc.slug}/${c.slug}`}
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">Other Contractors in {city.name}</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {otherCategories.map((c) => (
              <Link key={c.slug} href={`/uae/hire/${c.slug}/${loc.slug}/${city.slug}`}
      ...
      <section className="py-8 px-4 bg-brand-50 border-y border-brand-100">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-lg font-bold text-dark-900 mb-1">Check {cat.name} Costs in {city.name}</h3>
            <p className="text-sm text-dark-500">Get detailed cost breakdowns before hiring a contractor.</p>
          </div>
          <Link href="/uae/cost" className="flex-shrink-0 inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-semibold px-7 py-3.5 rounded-xl transition-colors">
            View Cost Guides <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </section>
