...
import { ChevronRight, MapPin, CheckCircle, ArrowRight, Info } from 'lucide-react';
...
export function generateStaticParams() {
  return UAE_COST_SERVICES.map(s => ({ service: s.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = svc.metaDesc.replace(/\{city\}/g, 'UAE');
  return {
    title, description: desc,
    keywords: [`${svc.name} cost UAE`, `${svc.name} rates 2026`, `${svc.name} cost per sq ft UAE`],
    alternates: { canonical: `https://biddaro.com/uae/cost/${svc.slug}` },
    openGraph: { title, description: desc, url: `https://biddaro.com/uae/cost/${svc.slug}`, type: 'website' },
  };
}

export default function UAECostServicePage({ params }: Props) {
  const svc = getUAECostService(params.service);
  if (!svc) notFound();
  const related = getRelatedUAECostServices(params.service);
  const topCities = UAE_LOCATIONS.flatMap(l => l.cities.slice(0, 2)).slice(0, 30);
  ...
  const schema = [
    ...
    { '@context': 'https://schema.org', '@type': 'FAQPage',
      mainEntity: svc.faqs.map(f => ({
        '@type': 'Question', name: f.q.replace(/\{city\}/g, 'UAE'),
        acceptedAnswer: { '@type': 'Answer', text: f.a.replace(/\{city\}/g, 'UAE') },
      })),
    },
  ];
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-dark-300 text-lg max-w-2xl mb-6">
            {svc.intro.replace(/\{city\}/g, 'the UAE')}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Link href={`/uae/hire/${svc.slug}`} className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-3.5 rounded-xl transition-colors border border-white/20">
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-start gap-2 mt-3 text-xs text-dark-400">
            ...
            Rates are indicative pan-UAE averages for 2026 in AED. Actual costs vary by city, material quality, and contractor.
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-dark-500 text-sm mb-8">Rates differ across the UAE \u2014 select your city for local pricing.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {topCities.map(city => (
              <Link key={city.slug} href={`/uae/cost/${svc.slug}/${city.slug}`}
        ...
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map(r => (
                <Link key={r.slug} href={`/uae/cost/${r.slug}`}
