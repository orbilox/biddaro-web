...
export default function ProjectDetail() {
  ...
  const saveDesc = async () => {
    ...
    setSavingDesc(true);
    try {
      await inspectApi.updateProject(project.id, { description: descDraft.trim() || null });
      setProject(prev => prev ? { ...prev, description: descDraft.trim() || null } : prev);
      setDescOpen(false);
        ...
        {descOpen ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <textarea
              ...
              onKeyDown={e => { if (e.key === 'Escape') setDescOpen(false); }}
              rows={4}
              placeholder="Add general observations, site access notes, or project background…"
              className="w-full text-sm border border-amber-300 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-amber-400 resize-none bg-white text-dark-800"
            />
            <div className="flex gap-2 mt-2">
              <button
                onClick={saveDesc}
                disabled={savingDesc}
                className="text-xs bg-amber-600 hover:bg-amber-700 text-white font-semibold px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-1"
              >
                {savingDesc && <Loader2 className="w-3 h-3 animate-spin" />} Save Notes
              </button>
              <button onClick={() => setDescOpen(false)} className="text-xs border border-dark-200 text-dark-600 px-3 py-1.5 rounded-lg hover:bg-dark-50">Cancel</button>
            </div>
          </div>
        ) : (
          <button
            onClick={() => { setDescOpen(true); setDescDraft(project.description ?? ''); }}
            className={`w-full text-left rounded-xl border px-4 py-3 text-sm transition-colors group hover:border-amber-300 hover:bg-amber-50/50 ${
              project.description ? 'border-amber-200 bg-amber-50' : 'border-dashed border-dark-200'
            }`}
          >
            <div className="flex items-start gap-2">
              <FileText className={`w-4 h-4 flex-shrink-0 mt-0.5 ${project.description ? 'text-amber-600' : 'text-dark-300'}`} />
              {project.description
                ? <p className="text-dark-700 leading-relaxed line-clamp-2">{project.description}</p>
                : <p className="text-dark-300 italic">Add project notes, site access info, or general observations…</p>
              }
              <Pencil className="w-3.5 h-3.5 text-dark-300 group-hover:text-amber-500 opacity-0 group-hover:opacity-100 transition-opacity ml-auto flex-shrink-0" />
            </div>
          </button>
        )}
      </div>
