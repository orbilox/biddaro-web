...
import { USA_JOB_CATEGORY_META, USA_LOCATIONS, getUSACategory } from '@/lib/seo-data-usa';
...
export function generateStaticParams() {
  return USA_JOB_CATEGORY_META.map((cat) => ({ category: cat.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!cat) return {};
  return {
    title: `${cat.plural} in USA – Find & Hire Top ${cat.name} Contractors | Biddaro`,
    description: `Connect with verified ${cat.plural.toLowerCase()} across all US states. Post your ${cat.name.toLowerCase()} job free on Biddaro and receive competitive bids within hours.`,
    keywords: [
      `${cat.plural.toLowerCase()} USA`,
      `hire ${cat.plural.toLowerCase()}`,
      `${cat.name.toLowerCase()} contractors near me`,
      'Biddaro contractors',
    ],
    alternates: { canonical: `https://biddaro.com/us/hire/${cat.slug}` },
    openGraph: {
      title: `${cat.plural} in USA | Biddaro`,
      description: `Find verified ${cat.plural.toLowerCase()} across the USA. Post free, get bids fast.`,
      url: `https://biddaro.com/us/hire/${cat.slug}`,
      type: 'website',
    },
  };
}

function JsonLd({ cat }: { cat: ReturnType<typeof getUSACategory> }) {
  if (!cat) return null;
  const schema = [
    {
    ...
    },
    {
      ...
      '@type': 'Service',
      name: cat.name,
      description: cat.longDesc.replace('{state}', 'USA'),
...
export default function USCategoryHubPage({ params }: Props) {
  const cat = getUSACategory(params.category);
  if (!cat) notFound();

  const regions = ['West', 'South', 'Southeast', 'Northeast', 'Midwest'];
    ...
    <>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-sm text-dark-500">
          ...
          <span>/</span>
          <Link href="/us/hire" className="hover:text-brand-600 transition-colors">Find Contractors</Link>
          <span>/</span>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-dark-300 text-lg max-w-2xl mb-8">
            {cat.longDesc.replace(/\{state\}/g, 'USA')}
          </p>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {USA_JOB_CATEGORY_META.filter((c) => c.slug !== cat.slug).map((c) => (
              <Link
                key={c.slug}
                href={`/us/hire/${c.slug}`}
