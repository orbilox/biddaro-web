...
import {
  ...
  getUSACategory, getUSALocationBySlug,
...
export function generateStaticParams() {
  return USA_JOB_CATEGORY_META.flatMap((cat) =>
    USA_LOCATIONS.map((loc) => ({ category: cat.slug, state: loc.slug }))
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `Find verified ${cat.plural.toLowerCase()} in ${loc.name}. Post your ${cat.name.toLowerCase()} job free on Biddaro and receive competitive bids within hours. Secure escrow payments included.`;
  const url = `https://biddaro.com/us/hire/${cat.slug}/${loc.slug}`;
...
function JsonLd({ cat, loc }: { cat: NonNullable<ReturnType<typeof getUSACategory>>; loc: NonNullable<ReturnType<typeof getUSALocationBySlug>> }) {
  const url = `https://biddaro.com/us/hire/${cat.slug}/${loc.slug}`;
  ...
  const schema = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 5, name: loc.name, item: url },
      ],
    },
    {
      '@context': 'https://schema.org',
      ...
      areaServed: { '@type': 'State', name: loc.name, containedInPlace: { '@type': 'Country', name: 'United States' } },
      url,
    },
    {
      '@context': 'https://schema.org',
...
function getDummyContractors(cat: NonNullable<ReturnType<typeof getUSACategory>>, loc: NonNullable<ReturnType<typeof getUSALocationBySlug>>) {
  const names = [
    'Liberty Construction Co.', 'Eagle Build Services', 'All-American Contractors',
    'Summit Home Solutions', 'Patriot Pro Builders', 'Pinnacle Construction',
    'Heritage Builders Inc.', 'Cornerstone Contractors', 'Blue Ridge Build Group', 'Pacific Coast Construction',
  ];
...
export default function USHireStatePage({ params }: Props) {
  const cat = getUSACategory(params.category);
  const loc = getUSALocationBySlug(params.state);
  if (!cat || !loc) notFound();

  const contractors = getDummyContractors(cat, loc);
  const otherCategories = USA_JOB_CATEGORY_META.filter((c) => c.slug !== cat.slug).slice(0, 10);
  const otherStates = USA_LOCATIONS.filter((l) => l.slug !== loc.slug).slice(0, 12);
            ...
            {[
              {
                ...
                desc: `Every ${cat.name.toLowerCase()} contractor in ${loc.name} is license-verified with work history reviewed before they can bid on your job.`,
              },
              {
                icon: Zap,
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {loc.cities.map((city) => (
              <Link
                key={city.slug}
                href={`/us/hire/${cat.slug}/${loc.slug}/${city.slug}`}
