...
export const metadata: Metadata = {
  title: 'Find Construction Contractors in USA | Biddaro',
  description:
    'Browse top-rated construction contractors, plumbers, electricians, painters and 20+ trade professionals across all US states. Post your job free on Biddaro.',
  keywords: [
    'construction contractors USA', 'hire contractors online USA',
    'plumbers near me USA', 'electricians USA', 'renovation contractors',
    'Biddaro marketplace USA',
  ],
  alternates: { canonical: 'https://biddaro.com/us/hire' },
  openGraph: {
    title: 'Find Top Construction Contractors in USA | Biddaro',
    description: 'Connect with 500+ verified contractors across all US states. Post a job free today.',
    url: 'https://biddaro.com/us/hire',
    type: 'website',
  },
};
  ...
  const schema = [
    {
      ...
      '@type': 'WebPage',
      name: 'Find Construction Contractors in USA',
      description: 'Browse contractors by category and location across the United States.',
      url: 'https://biddaro.com/us/hire',
    },
    {
      '@context': 'https://schema.org',
...
export default function USHireHubPage() {
  return (
    <>
      <JsonLd />
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-brand-500/20 border border-brand-500/30 text-brand-300 text-sm font-medium px-4 py-1.5 rounded-full mb-6">
            ...
            Trusted by 10,000+ homeowners across the USA
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-5 leading-tight">
            Find Top Construction Contractors<br className="hidden sm:block" />
            <span className="text-brand-400"> Near You in USA</span>
          </h1>
          <p className="text-lg text-dark-300 max-w-2xl mx-auto mb-8">
            Browse verified contractors by trade and location. Post your job free and receive competitive bids from licensed professionals in your area.
          </p>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="mb-10">
            ...
            <p className="text-dark-500 mt-2">Select a trade to see contractors available across all US states.</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {USA_JOB_CATEGORY_META.map((cat) => (
              <Link
                key={cat.slug}
                href={`/us/hire/${cat.slug}`}
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="mb-10">
            <h2 className="text-2xl md:text-3xl font-bold text-dark-900">Browse by State</h2>
            <p className="text-dark-500 mt-2">Find contractors available in your state across all trade categories.</p>
          </div>

          {['West', 'South', 'Southeast', 'Northeast', 'Midwest'].map((region) => {
            const states = USA_LOCATIONS.filter((l) => l.region === region);
              ...
              <div key={region} className="mb-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {states.map((state) => (
                    <Link
                      key={state.slug}
                      href={`/us/hire/general-construction/${state.slug}`}
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            {USA_JOB_CATEGORY_META.slice(0, 8).flatMap((cat) =>
              USA_LOCATIONS.filter((l) =>
                ['california', 'texas', 'florida', 'new-york', 'illinois', 'georgia'].includes(l.slug)
              ).map((state) => (
                <Link
                  key={`${cat.slug}-${state.slug}`}
                  href={`/us/hire/${cat.slug}/${state.slug}`}
