...
import { ChevronRight, MapPin, CheckCircle, IndianRupee, ArrowRight, Info } from 'lucide-react';
...
export function generateStaticParams() {
  return COST_SERVICES.map(s => ({ service: s.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = svc.metaDesc.replace(/\{city\}/g, 'India');
  return {
    title, description: desc,
    keywords: [`${svc.name} cost India`, `${svc.name} rates 2026`, `${svc.name} cost per sq ft India`],
    alternates: { canonical: `https://biddaro.com/cost/${svc.slug}` },
    openGraph: { title, description: desc, url: `https://biddaro.com/cost/${svc.slug}`, type: 'website' },
  };
}

export default function CostServicePage({ params }: Props) {
  const svc = getCostService(params.service);
  if (!svc) notFound();
  const related = getRelatedCostServices(params.service);
  const topCities = INDIA_LOCATIONS.flatMap(l => l.cities.slice(0, 2)).slice(0, 30);
  ...
  const schema = [
    ...
    { '@context': 'https://schema.org', '@type': 'FAQPage',
      mainEntity: svc.faqs.map(f => ({
        '@type': 'Question', name: f.q.replace(/\{city\}/g, 'India'),
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-dark-300 text-lg max-w-2xl mb-6">
            {svc.intro.replace(/\{city\}/g, 'India')}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Link href={`/hire/${svc.slug}`} className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-3.5 rounded-xl transition-colors border border-white/20">
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-start gap-2 mt-3 text-xs text-dark-400">
            ...
            Rates are indicative pan-India averages for 2026. Actual costs vary by city, material quality, and contractor.
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-dark-500 text-sm mb-8">Rates differ across India — select your city for local pricing.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {topCities.map(city => (
              <Link key={city.slug} href={`/cost/${svc.slug}/${city.slug}`}
        ...
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map(r => (
                <Link key={r.slug} href={`/cost/${r.slug}`}
      ...
      <section className="py-16 px-4 bg-gradient-to-r from-brand-500 to-brand-600">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-brand-100 mb-8">Post your job free on Biddaro. Get competing bids from verified contractors within 4 hours.</p>
