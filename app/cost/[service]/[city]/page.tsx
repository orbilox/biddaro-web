...
import { ChevronRight, MapPin, CheckCircle, IndianRupee, ArrowRight, Info, Star, TrendingUp } from 'lucide-react';
import { COST_SERVICES, getCostService, getRelatedCostServices } from '@/lib/cost-data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const svc = getCostService(params.service);
  ...
  return {
    ...
    alternates: { canonical: `https://biddaro.com/cost/${svc.slug}/${cityData.slug}` },
    openGraph: {
      title,
      description: desc,
      url: `https://biddaro.com/cost/${svc.slug}/${cityData.slug}`,
      type: 'website',
    },
  };
}
...
export default function CostCityPage({ params }: Props) {
  const svc = getCostService(params.service);
  const cityData = findCity(params.city);
  ...
  // Find matching hire category slug (same slug as cost service where possible)
  const hireLink = `/hire/${svc.slug}/${state.slug}/${cityData.slug}`;

  // Other cities in same state (for "nearby cities" section)
  const siblingCities = state.cities.filter(c => c.slug !== cityData.slug).slice(0, 6);

  // Other services for cross-linking
  const related = getRelatedCostServices(params.service);
  ...
  const schema = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: svc.name, item: `https://biddaro.com/cost/${svc.slug}` },
        { '@type': 'ListItem', position: 4, name: city, item: `https://biddaro.com/cost/${svc.slug}/${cityData.slug}` },
      ],
    },
    {
      '@context': 'https://schema.org',
      ...
      <nav className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="max-w-5xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-dark-500 flex-wrap">
            ...
            <li><Link href="/cost" className="hover:text-brand-600">Cost Guides</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li><Link href={`/cost/${svc.slug}`} className="hover:text-brand-600">{svc.name}</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
      ...
      <div className="bg-white border-b border-gray-100 py-4 px-4">
        <div className="max-w-5xl mx-auto flex flex-wrap items-center justify-center gap-6 text-sm text-dark-500">
          {[
            ...
            { icon: <IndianRupee className="w-4 h-4 text-brand-500" />, text: 'Free to post job' },
          ].map((item, i) => (
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-dark-500 text-sm mb-6">
            Indicative rates based on recent quotes from contractors in {city} and {state.name}.
          ...
          <div className="flex items-start gap-2 mt-3 text-xs text-dark-400">
            ...
            Rates are indicative averages for {city} in {yr}. Post a job on Biddaro to get competitive quotes from verified local contractors.
          </div>
        </div>
      </section>
        ...
        <section className="py-12 px-4 bg-white">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map(r => (
                <Link key={r.slug} href={`/cost/${r.slug}/${cityData.slug}`}
                  ...
                  <p className="text-xs text-dark-400 mt-0.5">{r.avgLow}–{r.avgHigh}</p>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="py-8 px-4 bg-gray-50 border-t border-gray-100">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/cost" className="text-sm font-semibold text-brand-600 hover:text-brand-700 underline underline-offset-2">
              Browse All Cost Guides →
            </Link>
            <Link href={`/hire/${svc.slug}/${state.slug}/${cityData.slug}`}
