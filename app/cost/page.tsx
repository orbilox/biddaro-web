...
import { ChevronRight, TrendingUp, Calculator, IndianRupee } from 'lucide-react';
import { COST_SERVICES } from '@/lib/cost-data';
import { JOB_CATEGORY_META, INDIA_LOCATIONS } from '@/lib/seo-data';

export const metadata: Metadata = {
  title: 'Construction Cost Guide India 2026 – Rates & Estimates | Biddaro',
  description: 'Accurate construction cost guides for India 2026. Find out how much house construction, plumbing, painting, flooring, electrical work costs in your city.',
  keywords: ['construction cost India', 'house construction cost per sq ft', 'plumber rates India', 'painting cost India', 'renovation cost India 2026'],
  alternates: { canonical: 'https://biddaro.com/cost' },
  openGraph: {
    title: 'Construction Cost Guide India 2026 | Biddaro',
    description: 'Detailed cost breakdowns for every construction service across India.',
    url: 'https://biddaro.com/cost', type: 'website',
  },
};
    ...
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([
      { '@context': 'https://schema.org', '@type': 'BreadcrumbList',
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Cost Guides', item: 'https://biddaro.com/cost' },
        ],
      },
      { '@context': 'https://schema.org', '@type': 'WebPage',
        name: 'Construction Cost Guide India 2026',
        description: 'Comprehensive construction cost guide for all Indian cities and states.',
        url: 'https://biddaro.com/cost',
      },
    ]) }} />
  );
}
...
export default function CostHubPage() {
  const topCities = INDIA_LOCATIONS
    .filter(l => ['maharashtra','delhi','karnataka','tamil-nadu','gujarat','telangana','rajasthan','west-bengal'].includes(l.slug))
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-20 px-4">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-5 leading-tight">
            ...
            <span className="text-brand-400">India 2026</span>
          </h1>
          <p className="text-lg text-dark-300 max-w-2xl mx-auto mb-8">
            Accurate, city-specific cost breakdowns for every construction service in India.
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link href="/hire" className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-3.5 rounded-xl transition-colors border border-white/20">
              Find Contractors
            </Link>
          </div>
        </div>
      ...
      <section className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-3 gap-6 text-center">
          {[
            { value: `${COST_SERVICES.length}+`, label: 'Cost Guides' },
            { value: '500+', label: 'Indian Cities' },
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {COST_SERVICES.map(service => (
              <Link key={service.slug} href={`/cost/${service.slug}`}
                ...
                <p className="text-xs text-dark-500 mb-3 line-clamp-2">{service.metaDesc.replace(/ \{city\}/g, ' India')}</p>
      ...
      {/* Internal links — all service × city combos for SEO */}
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">Popular Cost Searches</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-1.5">
            {COST_SERVICES.slice(0, 5).flatMap(s =>
              INDIA_LOCATIONS
                .filter(l => ['maharashtra','delhi','karnataka','tamil-nadu','gujarat'].includes(l.slug))
                .flatMap(l => l.cities.slice(0, 2).map(city => (
                  <Link key={`${s.slug}-${city.slug}`} href={`/cost/${s.slug}/${city.slug}`}
