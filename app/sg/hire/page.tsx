...
export const metadata: Metadata = {
  title: 'Find Construction Contractors in Singapore | Biddaro',
  description:
    'Browse top-rated construction contractors, plumbers, electricians, painters and 15+ trade professionals across all Singapore regions. Post your job free on Biddaro.',
  keywords: [
    'construction contractors Singapore', 'hire contractors online Singapore',
    'plumbers near me Singapore', 'electricians Singapore', 'renovation contractors Singapore',
    'Biddaro marketplace Singapore', 'BCA licensed contractors',
  ],
  alternates: { canonical: 'https://biddaro.com/sg/hire' },
  openGraph: {
    title: 'Find Top Construction Contractors in Singapore | Biddaro',
    description: 'Connect with 500+ verified contractors across all Singapore regions. Post a job free today.',
    url: 'https://biddaro.com/sg/hire',
    type: 'website',
  },
};
  ...
  const schema = [
    {
      ...
      '@type': 'WebPage',
      name: 'Find Construction Contractors in Singapore',
      description: 'Browse contractors by category and location across Singapore.',
      url: 'https://biddaro.com/sg/hire',
    },
    {
      '@context': 'https://schema.org',
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Find Contractors', item: 'https://biddaro.com/sg/hire' },
      ],
    },
  ];
  return (
...
export default function SGHireHubPage() {
  return (
    <>
      <JsonLd />
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-5 leading-tight">
            ...
            <span className="text-brand-400"> in Singapore</span>
          </h1>
          <p className="text-lg text-dark-300 max-w-2xl mx-auto mb-8">
            Browse BCA-registered contractors by trade and location. Post your job free and receive competitive bids from professionals in your area.
          </p>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="mb-10">
            ...
            <p className="text-dark-500 mt-2">Select a trade to see contractors available across all Singapore regions.</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {SG_JOB_CATEGORY_META.map((cat) => (
              <Link
                key={cat.slug}
                href={`/sg/hire/${cat.slug}`}
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="mb-10">
            <h2 className="text-2xl md:text-3xl font-bold text-dark-900">Browse by Region</h2>
            <p className="text-dark-500 mt-2">Find contractors available in your region across all trade categories.</p>
          </div>

          {SG_LOCATIONS.map((loc) => (
            <div key={loc.slug} className="mb-10">
              <h3 className="text-base font-semibold text-dark-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                <MapPin className="w-4 h-4 text-brand-500" /> {loc.name}
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {SG_JOB_CATEGORY_META.slice(0, 5).map((cat) => (
                  <Link
                    key={`${cat.slug}-${loc.slug}`}
                  ...
                  >
                    <div>
                      <p className="text-sm font-semibold text-dark-800 group-hover:text-brand-700">{cat.name}</p>
                      <p className="text-xs text-dark-400">{loc.districts.length} districts</p>
                    </div>
      ...
      {/* ── Internal links grid (All category x All region combos) ────────────── */}
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-bold text-dark-900 mb-2">Popular Contractor Searches</h2>
          <p className="text-dark-500 mb-8">Explore our most-searched contractor categories by region.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            {SG_JOB_CATEGORY_META.slice(0, 8).flatMap((cat) =>
              SG_LOCATIONS.map((loc) => (
                <Link
                  key={`${cat.slug}-${loc.slug}`}
                  href={`/sg/hire/${cat.slug}/${loc.slug}`}
