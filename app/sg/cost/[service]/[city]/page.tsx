...
function getAllDistricts() {
  return SG_LOCATIONS.flatMap(region =>
    region.districts.map(district => ({ ...district, region }))
  );
}

function findDistrict(slug: string) {
  ...
  for (const svc of SG_COST_SERVICES) {
    for (const region of SG_LOCATIONS) {
      for (const district of region.districts) {
        params.push({ service: svc.slug, city: district.slug });
      }
    }
  }
  return params;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const districtData = findDistrict(params.city);
  if (!svc || !districtData) return {};

  const district = districtData.name;
  const region = districtData.region.name;
  const title = `${svc.name} Cost in ${district} ${new Date().getFullYear()} – Rates & Estimates | Biddaro`;
  const desc = svc.metaDesc.replace(/\{city\}/g, district);

  return {
    title,
    description: desc,
    keywords: [
      `${svc.name.toLowerCase()} cost in ${district}`,
      `${svc.name.toLowerCase()} rates ${district}`,
      `${svc.name.toLowerCase()} charges ${district} ${new Date().getFullYear()}`,
      `${svc.name.toLowerCase()} price per sqft ${district}`,
      `best ${svc.name.toLowerCase()} contractors ${district}`,
      `${svc.name.toLowerCase()} cost ${region} Singapore`,
    ],
    alternates: { canonical: `https://biddaro.com/sg/cost/${svc.slug}/${districtData.slug}` },
    openGraph: {
      title,
      description: desc,
      url: `https://biddaro.com/sg/cost/${svc.slug}/${districtData.slug}`,
      type: 'website',
    },
  };
}

export default function SGCostDistrictPage({ params }: Props) {
  const svc = getSGCostService(params.service);
  const districtData = findDistrict(params.city);
  if (!svc || !districtData) notFound();

  const district = districtData.name;
  const region = districtData.region;
  const yr = new Date().getFullYear();
  const r = (str: string) => str.replace(/\{city\}/g, district);

  const hireLink = `/sg/hire/${svc.slug}/${region.slug}/${districtData.slug}`;
  const siblingDistricts = region.districts.filter(d => d.slug !== districtData.slug).slice(0, 6);
  ...
  const schema = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 4, name: district, item: `https://biddaro.com/sg/cost/${svc.slug}/${districtData.slug}` },
      ],
    },
    {
      '@context': 'https://schema.org',
      '@type': 'Service',
      name: `${svc.name} in ${district}`,
      description: r(svc.metaDesc),
      provider: { '@type': 'Organization', name: 'Biddaro', url: 'https://biddaro.com' },
      areaServed: { '@type': 'Place', name: district, containedInPlace: { '@type': 'AdministrativeArea', name: region.name, containedInPlace: { '@type': 'Country', name: 'Singapore' } } },
  ...
  return (
    <>
      ...
      <nav className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="max-w-5xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-dark-500 flex-wrap">
            ...
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li><Link href={`/sg/cost/${svc.slug}`} className="hover:text-brand-600">{svc.name}</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li className="text-dark-800 font-medium">{district}</li>
          </ol>
        </div>
      </nav>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-brand-300 text-sm font-medium">{district}, {region.name}</span>
          </div>
          <div className="text-4xl mb-4">{svc.emoji}</div>
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">
            {svc.name} Cost in {district} {yr}
          </h1>
          ...
          <div className="flex items-center gap-3 bg-brand-500/20 border border-brand-500/30 rounded-xl px-4 py-3 inline-flex mb-8">
            ...
            <span className="text-dark-400 text-sm">{svc.avgUnit} in {district}</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href="/register" className="inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-semibold px-8 py-3.5 rounded-xl transition-colors">
              Get Free Quotes in {district} <ArrowRight className="w-4 h-4" />
            </Link>
      ...
      <div className="bg-white border-b border-gray-100 py-4 px-4">
        <div className="max-w-5xl mx-auto flex flex-wrap items-center justify-center gap-6 text-sm text-dark-500">
          {[
            ...
            { icon: <CheckCircle className="w-4 h-4 text-green-500" />, text: 'BCA-verified professionals' },
            { icon: <TrendingUp className="w-4 h-4 text-brand-500" />, text: `${yr} updated rates` },
            { icon: <span className="text-brand-500 font-bold text-xs">S$</span>, text: 'Free to post job' },
          ].map((item, i) => (
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold text-dark-900 mb-2">
            {svc.name} Cost Breakdown in {district} {yr}
          </h2>
          <p className="text-dark-500 text-sm mb-6">
            Indicative rates based on recent quotes from contractors in {district}, {region.name}.
          ...
          <div className="flex items-start gap-2 mt-3 text-xs text-dark-400">
            ...
            Rates are indicative averages for {district} in {yr} (SGD). Post a job on Biddaro to get competitive quotes from verified local contractors.
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">
            Factors Affecting {svc.name} Cost in {district}
          </h2>
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">
            How to Save Money on {svc.name} in {district}
          </h2>
        ...
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-lg font-bold text-dark-900 mb-1">
              Ready to hire a {svc.name} contractor in {district}?
            </h3>
            <p className="text-sm text-dark-500">
              Post your job free on Biddaro. Get competing bids from {district}&apos;s top BCA-verified contractors within 4 hours.
            </p>
          </div>
          <Link href={hireLink}
            ...
            Find {svc.name} Contractors in {district} <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </section>
        ...
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-xl font-bold text-dark-900 mb-2">
              {svc.name} Cost in Other Areas of {region.name}
            </h2>
            <p className="text-sm text-dark-500 mb-6">Compare rates across major districts in {region.name}.</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
              {siblingDistricts.map(d => (
                <Link key={d.slug} href={`/sg/cost/${svc.slug}/${d.slug}`}
      ...
      {related.length > 0 && (
        <section className="py-12 px-4 bg-white">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-xl font-bold text-dark-900 mb-6">
              Other Construction Costs in {district}
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map(rel => (
                <Link key={rel.slug} href={`/sg/cost/${rel.slug}/${districtData.slug}`}
                  ...
                  <p className="text-xs text-brand-600 mt-1">in {district}</p>
      ...
      <section className="py-8 px-4 bg-gray-50 border-t border-gray-100">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-dark-500">
            Want a complete picture of all construction costs in <span className="font-semibold text-dark-800">{district}</span>?
          </p>
          <div className="flex flex-wrap gap-3">
            <Link href="/sg/cost" className="text-sm font-semibold text-brand-600 hover:text-brand-700 underline underline-offset-2">
              Browse All SG Cost Guides &rarr;
            </Link>
            <Link href={hireLink}
              className="text-sm font-semibold text-brand-600 hover:text-brand-700 underline underline-offset-2">
              Hire in {district} &rarr;
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-gradient-to-r from-brand-500 to-brand-600">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl font-bold text-white mb-4">
            Get Accurate {svc.name} Quotes in {district}
          </h2>
          <p className="text-brand-100 mb-8">
            Post your job free on Biddaro. Verified {district} contractors bid competitively — you save 15–30% on average.
          </p>
          <Link href="/register"
