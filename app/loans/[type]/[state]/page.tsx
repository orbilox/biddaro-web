...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://biddaro.com/loans/${loan.slug}/${state.slug}` },
    openGraph: {
      title,
      description: desc,
      url: `https://biddaro.com/loans/${loan.slug}/${state.slug}`,
      type: 'website',
    },
  };
}
...
export default function LoanStateHubPage({ params }: Props) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: state.name,         item: `https://biddaro.com/loans/${loan.slug}/${state.slug}` },
    ],
  };
