...
// ── Static params — 21 categories × 31 states = 651 static pages ─────────────
export function generateStaticParams() {
  return JOB_CATEGORY_META.flatMap((cat) =>
    INDIA_LOCATIONS.map((loc) => ({ category: cat.slug, state: loc.slug }))
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `Find verified ${cat.plural.toLowerCase()} in ${loc.name}, ${loc.region}. Post your ${cat.name.toLowerCase()} job free on Biddaro and receive competitive bids within hours. Secure escrow payments included.`;
  const url = `https://biddaro.com/hire/${cat.slug}/${loc.slug}`;
...
// ── JSON-LD schemas ────────────────────────────────────────────────────────────
function JsonLd({ cat, loc }: { cat: NonNullable<ReturnType<typeof getCategory>>; loc: NonNullable<ReturnType<typeof getLocation>> }) {
  const url = `https://biddaro.com/hire/${cat.slug}/${loc.slug}`;
...
// ── Dummy contractor data generator ───────────────────────────────────────────
function getDummyContractors(cat: NonNullable<ReturnType<typeof getCategory>>, loc: NonNullable<ReturnType<typeof getLocation>>) {
  const names = [
    'Rajan Construction', 'Sharma Builders', 'Gupta & Sons Contractors',
    'Mehta Pro Services', 'Singh & Associates', 'Reddy Build Masters',
    'Patel Works Pvt Ltd', 'Kumar Infrastructure', 'Nair Home Solutions', 'Verma Contractors',
  ];
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function HireStatePage({ params }: Props) {
  const cat = getCategory(params.category);
  const loc = getLocation(params.state);
  if (!cat || !loc) notFound();

  const contractors = getDummyContractors(cat, loc);
  const otherCategories = JOB_CATEGORY_META.filter((c) => c.slug !== cat.slug).slice(0, 10);
  const otherStates = INDIA_LOCATIONS.filter((l) => l.slug !== loc.slug).slice(0, 12);
  ...
  return (
    <>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-3" aria-label="Breadcrumb">
        <div className="max-w-7xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-dark-500 flex-wrap">
            ...
            <li><Link href="/hire" className="hover:text-brand-600 transition-colors">Find Contractors</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li><Link href={`/hire/${cat.slug}`} className="hover:text-brand-600 transition-colors">{cat.plural}</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
      ...
      <section className="bg-gradient-to-br from-dark-900 via-dark-800 to-brand-900 py-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-brand-500/20 border border-brand-500/30 text-brand-300 text-xs font-medium px-3 py-1.5 rounded-full mb-5">
            <MapPin className="w-3 h-3" /> {loc.name}, {loc.region}
          </div>
      ...
      <section className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            ...
            { icon: DollarSign, value: '₹0', label: 'To Post a Job' },
            ...
            {[
              {
                ...
                desc: `Every ${cat.name.toLowerCase()} contractor in ${loc.name} is ID-verified with work history reviewed before they can bid on your job.`,
              },
              {
                icon: Zap,
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-8">
            {[
              ...
              { step: '02', title: 'Receive Bids', desc: `Verified ${cat.plural.toLowerCase()} in ${loc.name} review your job and send their best quotes within hours.` },
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-6 p-4 bg-brand-50 border border-brand-100 rounded-xl text-sm text-brand-800">
            <strong>Average Rate in {loc.name}:</strong> {cat.avgRate} — rates may vary based on job complexity and location within the state.
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-2">
            {cat.plural} by City in {loc.name}
          </h2>
          <p className="text-dark-500 text-sm mb-6">
            Find {cat.plural.toLowerCase()} in specific cities across {loc.name}.
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {loc.cities.map((city) => (
              <Link
                key={city.slug}
                href={`/hire/${cat.slug}/${loc.slug}/${city.slug}`}
      ...
      {/* ── Internal links: Other categories in this state ───────────────── */}
      <section className="py-12 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">
      ...
      {/* ── Internal links: Same category in other states ────────────────── */}
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-bold text-dark-900 mb-6">
            {cat.plural} in Other States
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {otherStates.map((s) => (
              <Link
                key={s.slug}
                href={`/hire/${cat.slug}/${s.slug}`}
      ...
      <section className="py-16 px-4 bg-gradient-to-r from-brand-500 to-brand-600">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-brand-200 text-sm mt-4">
            No commission charged to job posters • Secure escrow included
          </p>
        </div>
      </section>
    </>
