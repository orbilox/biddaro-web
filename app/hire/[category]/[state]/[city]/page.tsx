...
import {
  ...
  getCategory, getLocation, getCity,
} from '@/lib/seo-data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://biddaro.com/hire/${cat.slug}/${loc.slug}/${city.slug}`;
...
function JsonLd({
  ...
  loc: NonNullable<ReturnType<typeof getLocation>>;
  ...
  const schema = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 4, name: loc.name, item: `https://biddaro.com/hire/${cat.slug}/${loc.slug}` },
...
function getDummyContractors(
  ...
  return [
    { id: 1, name: `${cityName} Build Masters`, rating: 4.9, reviews: 87, jobs: 156, verified: true, specialty: cat.skills[0] },
    { id: 2, name: `Sharma ${cat.name} Experts`, rating: 4.7, reviews: 52, jobs: 93, verified: true, specialty: cat.skills[1 % cat.skills.length] },
    ...
    { id: 5, name: `Gupta & Sons Contractors`, rating: 4.8, reviews: 71, jobs: 118, verified: true, specialty: cat.skills[4 % cat.skills.length] },
    { id: 6, name: `Quick Fix ${cat.name}`, rating: 4.4, reviews: 19, jobs: 32, verified: false, specialty: cat.skills[0] },
  ];
}

export default function HireCityPage({ params }: Props) {
  const cat = getCategory(params.category);
  const loc = getLocation(params.state);
  const city = getCity(params.state, params.city);
  ...
  const otherCategories = JOB_CATEGORY_META.filter((c) => c.slug !== cat.slug).slice(0, 8);
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-3" aria-label="Breadcrumb">
        <div className="max-w-7xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-dark-500 flex-wrap">
            ...
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
            <li><Link href={`/hire/${cat.slug}/${loc.slug}`} className="hover:text-brand-600 transition-colors">{loc.name}</Link></li>
            <li><ChevronRight className="w-3.5 h-3.5" /></li>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {otherCities.map((c) => (
                <Link
                  key={c.slug}
                  href={`/hire/${cat.slug}/${loc.slug}/${c.slug}`}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {otherCategories.map((c) => (
              <Link
                key={c.slug}
                href={`/hire/${c.slug}/${loc.slug}/${city.slug}`}
